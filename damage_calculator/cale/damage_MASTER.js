...
function addLevelDesc(attacker, defender, description) {
    autoLevel = _calcConfig.isDoubleBattle ? 50 : 100;
...
function checkParadoxAbilities(pokemon, terrain, weather) {
    if (['Protosynthesis', 'Quark Drive'].indexOf(pokemon.ability) !== -1) {
        if ((pokemon.ability === 'Protosynthesis' && weather === 'Sun')
            ...
            || (typeof manualProtoQuark !== 'undefined' && manualProtoQuark && pokemon.item !== 'Booster Energy'))
...
function changeStatBoosts(interactionArray, stat, numStages) {
    ...
    if (numStages < 0) {
        ...
        console.warn("Entered changeStatBoosts with zero stat changes");
    }
...
function checkEvo(p1, p2) {
    ...
    if (_calcConfig.evoBoostL || _calcConfig.tatsuL){
    ...
    if (_calcConfig.evoBoostR || _calcConfig.tatsuR){
...
function ZMoves(move, field, attacker, moveDescName) {
    if (move.isSignatureZ) {
        ...
        if (attacker.isTransformed) {
            var tempSpecies = _calcConfig.isTransformedL ? transformSpecies["p1"] : transformSpecies["p2"];
...
function calcBPMods(attacker, defender, field, move, description, ateIzeBoosted, basePower, attIsGrounded, defIsGrounded, turnOrder, defAbility, isMeFirst) {
    ...
    var auraActive = (_calcConfig.auraChecked ? _calcConfig.auraChecked[move.type.toLowerCase()] : false);
    var auraBreak = (_calcConfig.auraBreakChecked || false);
...
function calcAtMods(move, attacker, defAbility, description, field) {
    atMods = [];
    var ruinActive = {
        "Tablets of Ruin": (_calcConfig.tabletsOfRuin || false) && !field.isNeutralizingGas,
        "Vessel of Ruin": (_calcConfig.vesselOfRuin || false) && !field.isNeutralizingGas,
    };
...
function calcDefMods(move, defender, field, description, hitsPhysical, defAbility) {
    var dfMods = [];
    var ruinActive = {
        "Sword of Ruin": (_calcConfig.swordOfRuin || false) && !field.isNeutralizingGas,
        "Beads of Ruin": (_calcConfig.beadsOfRuin || false) && !field.isNeutralizingGas,
    };
