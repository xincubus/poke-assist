...
var ITEMNAMES = {
    ...
    "Rawst Berry": "莓莓果(解烧伤)",
    "Aspear Berry": "利木果(解冻)",
    "Leppa Berry": "苹野果(回复10PP)",
    "Oran Berry": "橙橙果(回复10PP)",
    "Persim Berry": "柿仔果(解混乱)",
    "Lum Berry": "木子果(解异常)",
    "Sitrus Berry": "文柚果(回复1/4)",
    "Figy Berry": "勿花果(危机回复1/3)",
    "Wiki Berry": "异奇果(危机回复1/3)",
    "Mago Berry": "芒芒果(危机回复1/3)",
    "Aguav Berry": "乐芭果(危机回复1/3)",
    "Iapapa Berry": "芭亚果(危机回复1/3)",
    "Razz Berry": "蔓莓果",
    "Bluk Berry": "墨莓果",
    "Nanab Berry": "蕉香果",
    "Wepear Berry": "西梨果",
    ...
    "Durin Berry": "金枕果",
    "Belue Berry": "靛莓果",
    "Liechi Berry": "枝荔果(危机+攻)",
    "Ganlon Berry": "龙睛果(危机+防)",
    "Salac Berry": "沙鳞果(危机+速)",
    "Petaya Berry": "龙火果(危机+特攻)",
    "Apicot Berry": "杏仔果(危机+特防)",
    "Lansat Berry": "兰萨果(危机+要害)",
    "Starf Berry": "星桃果(危机随机+2)",
    "Enigma Berry": "谜芝果(被效果绝佳回复1/4)",
    "Pumkin Berry": "瓜南果",
    "Drash Berry": "无青果",
    "Eggant Berry": "子茄果",
    "Strib Berry": "宛豆果",
    ...
    "Colbur Berry": "刺耳果(抗恶)",
    "Babiri Berry": "霹霹果(抗钢)",
    "Micle Berry": "奇秘果(危机+命中)",
    "Custap Berry": "释陀果(危机先制)",
    "Jaboca Berry": "嘉珍果(物理反伤1/8HP)",
    "Rowap Berry": "雾莲果(特殊反伤1/8HP)",
    "Roseli Berry": "洛玫果(抗仙)",
    "Kee Berry": "亚开果(受物攻+1防)",
    "Maranga Berry": "香罗果(受特攻+1特防)",
    "Coin Case": "代币盒",
    "Dowsing Machine": "探宝器",
    "Itemfinder": "探宝器",
    "Dowsing MCHN": "探宝器",
...
function translate_pokemon(name) {
    ...
    if (idx == 0) {
        let name0 = name.substring('Mega'.length + 1);
        // 处理 Mega X / Mega Y 形态：拆分尾部 " X" / " Y"
        let suffix = '';
        if (name0.endsWith(' X')) {
            suffix = ' X';
            name0 = name0.substring(0, name0.length - 2);
        } else if (name0.endsWith(' Y')) {
            suffix = ' Y';
            name0 = name0.substring(0, name0.length - 2);
        }
        return '超级' + (translate_pokemon(name0)) + suffix;
    }
    ...
    idx = name.indexOf('-Hisui');
    if (idx != -1) {
        let name0 = name.substring(0, idx);
        return translate_pokemon(name0) + '-洗翠'
    }
    ...
    formes = [{
        ...
        name: 'Necrozma',
        types: {
            'Solgaleo': '黄昏之鬃', 'Dusk-Mane': '黄昏之鬃',
            'Lunala': '拂晓之翼', 'Dawn-Wings': '拂晓之翼',
            'Ultra': '究极',
        }
    }, {
        name: 'Eiscue',
        ...
        name: 'Dudunsparce',
        types: {
            'Big': '三节形态', 'Three-Segment': '三节形态',
        }
    }, {
        name: 'Gimmighoul',
        types: {
            'Roam': '徒步形态', 'Roaming': '徒步形态',
        }
    }, {
        name: 'Maushold',
        types: {
        ...
        name: 'Tauros',
        types: {
            'Paldea': '帕底亚', 'Paldea-Combat': '帕底亚-斗战种',
            'Paldea-Water': '帕底亚-水澜种', 'Paldea-Aqua': '帕底亚-水澜种',
            'Paldea-Fire': '帕底亚-火炽种', 'Paldea-Blaze': '帕底亚-火炽种',
        }
    }, {
        name: 'Wooper',
        types: {
    ...
    if (idx != -1) {
        for (let forme of formes) {
            if (name.indexOf(forme.name) == 0) {
                ...
                let translated = forme.types[type];
                return POKENAMES[forme.name] + '-' + (translated != null ? translated : type);
            }
        }
    }
