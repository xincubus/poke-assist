...
[QueryProperty(nameof(PendingUrl), "url")]
public partial class DamageCalcPage : ContentPage
{
	...
	public DamageCalcPage(PokemonStorageService storageService, PokemonDbService db)
	{
	...
	private async Task SavePokemonFromCalcAsync(int pnum)
	{
		try
		{
			...
			var match = searchResults.FirstOrDefault(r =>
				...
				|| string.Equals(r.NameNcp, nameEn, StringComparison.OrdinalIgnoreCase));
			if (match != null)
			{
