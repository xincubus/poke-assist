"""批量运行 status 同步检测（mimo-v2.5 thinking）

用法：
  python run_sync_status.py --since 2026-04-01
  python run_sync_status.py --file pokemon_data/wiki/wikitext_cache/123_灼伤（状态）.wiki
"""
...
def find_stale_by_file(file_path):
    ...
    # 状态表：通过 name_zh 匹配
    db_row = conn.execute(
        "SELECT * FROM status WHERE name_zh = ? OR name_zh = ?",
        (wiki_title, wiki_title.replace('（状态）', ''))
    ).fetchone()
    if not db_row:
        db_row = conn.execute(
            "SELECT * FROM status WHERE wiki_file_path LIKE ?", (f'%{basename}',)
        ).fetchone()
    if not db_row:
        import re
        if os.path.exists(file_path):
            ...
            if m:
                ...
                db_row = conn.execute("SELECT * FROM status WHERE name_zh = ?", (name,)).fetchone()
    if not db_row:
        conn.close()
        return None

    columns = [c[1] for c in conn.execute("PRAGMA table_info(status)")]
...
def main():
    parser = argparse.ArgumentParser(description="运行 status 同步检测")
    ...
    if args.file:
        ...
        suggestions = llm_extract_changes([stale], 'status', POKEMON_DB)
        ...
        out = os.path.join(os.path.dirname(__file__), 'sync_reports', 'sync_report_status_single.json')
    ...
    # Phase 1: 新条目检测
    print(f'Detecting new status...', flush=True)
    new_entries, renamed_entries = detect_new_entries('status', pages, redirects, reverse_redirects, POKEMON_DB)
    ...
    new_items_info = []
    if new_entries:
        print(f'LLM extracting new status info...', flush=True)
        for i, entry in enumerate(new_entries):
            wiki_title = entry.get('wiki_title', '')
            print(f'  [{i+1}/{len(new_entries)}] {entry["name_zh"]}...', flush=True)
            info = llm_extract_entity_info(wiki_title, 'status')
            if info:
    ...
    if not args.new_only:
        print(f'Detecting stale status (since {args.since}, until {args.until or "now"})...', flush=True)
        stale = detect_stale_entries('status', pages, redirects, POKEMON_DB, args.since, args.until)
    ...
    out = os.path.join(os.path.dirname(__file__), 'sync_reports', 'sync_report_status_mimo.json')
    os.makedirs(os.path.dirname(out), exist_ok=True)
    with open(out, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=2)
    md_path = generate_summary_md(report, out)
