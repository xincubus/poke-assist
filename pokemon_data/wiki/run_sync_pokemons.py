...
用法：
  python run_sync_pokemons.py --since 2026-04-01
  python run_sync_pokemons.py --file pokemon_data/wiki/wikitext_cache/159_喷火龙.wiki
"""
...
def find_stale_by_file(file_path):
    """单文件模式：用 page_id 从 wiki_meta.db 查 title，从 pokemons 表查条目"""
    ...
    # 用 wiki_file_path 中的 page_id 匹配所有形态
    db_rows = conn.execute(
        "SELECT * FROM pokemons WHERE wiki_file_path LIKE ?", (f'%{page_id}_%',) if page_id else (f'%{basename}',)
    ).fetchall()
    if not db_rows:
        # 尝试 name_zh 匹配
        db_rows = conn.execute(
            "SELECT * FROM pokemons WHERE name_zh = ?", (wiki_title,)
        ).fetchall()
    if not db_rows:
        conn.close()
        return None

    columns = [c[1] for c in conn.execute("PRAGMA table_info(pokemons)")]
    all_entries = [{col: r[col] for col in columns} for r in db_rows]
    # 取 is_default_form=1 的条目作为代表
    default = next((e for e in all_entries if e.get("is_default_form")), all_entries[0])
    conn.close()
    return {
        ...
        "summary": "",
        "db_entry": default,
        "db_entries": all_entries,  # 所有形态
        "form_count": len(all_entries),
        "file_path": file_path,
    }

def main():
    parser = argparse.ArgumentParser(description="运行 pokemons 同步检测")
    ...
    if args.file:
        ...
        suggestions = llm_extract_changes([stale], 'pokemons', POKEMON_DB)
        ...
        out = os.path.join(os.path.dirname(__file__), 'sync_reports', f'sync_report_pokemons_{timestamp}.json')
    ...
    # Phase 1: 新条目检测
    print(f'Detecting new pokemons...', flush=True)
    new_entries, renamed_entries = detect_new_entries('pokemons', pages, redirects, reverse_redirects, POKEMON_DB)
    ...
    new_items_info = []
    if new_entries:
        print(f'LLM extracting new pokemons info...', flush=True)
        for i, entry in enumerate(new_entries):
            wiki_title = entry.get('wiki_title', '')
            print(f'  [{i+1}/{len(new_entries)}] {entry["name_zh"]}...', flush=True)
            info = llm_extract_entity_info(wiki_title, 'pokemons')
            if info:
    ...
    if not args.new_only:
        print(f'Detecting stale pokemons (since {args.since}, until {args.until or "now"})...', flush=True)
        stale = detect_stale_entries('pokemons', pages, redirects, POKEMON_DB, args.since, args.until)
