"""Apply abilities sync changes from report to pokemonData.db"""
...
def find_latest_report():
    """找到最新的 abilities 同步报告"""
    pattern = os.path.join(SYNC_REPORTS_DIR, 'sync_report_abilities_*.json')
...
def apply_changes(report_path, dry_run=False):
    ...
    for entry in changes:
        ...
        for change in entry['changes']:
            ...
            cur.execute(f'UPDATE abilities SET {field} = ? WHERE id = ?', (new_val, ability_id))
    ...
    conn.commit()
    conn.close()
    print(f'\nCommitted {updated} abilities ({skipped} skipped)')
...
if __name__ == '__main__':
    ...
    if not report_path:
        ...
        if not report_path:
            print('未找到报告文件，请指定路径：python apply_sync_abilities.py <report.json>')
            sys.exit(1)
