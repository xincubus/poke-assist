"""批量运行 items 同步检测（mimo-v2.5 thinking）

用法：
  python run_sync_items.py --since 2026-04-01
  python run_sync_items.py --file pokemon_data/wiki/wikitext_cache/302775_炎武王进化石（道具）.wiki
"""
...
def find_stale_by_file(file_path):
    ...
    # 道具表：通过 name_zh 或 wiki_file_path 匹配
    db_row = conn.execute(
        "SELECT * FROM items WHERE name_zh = ? OR name_zh = ?",
        (wiki_title, wiki_title.replace('（道具）', ''))
    ).fetchone()
    if not db_row:
        db_row = conn.execute(
            "SELECT * FROM items WHERE wiki_file_path LIKE ?", (f'%{basename}',)
        ).fetchone()
    if not db_row:
        import re
        if os.path.exists(file_path):
            ...
            if m:
                ...
                db_row = conn.execute("SELECT * FROM items WHERE name_zh = ?", (name,)).fetchone()
    if not db_row:
        conn.close()
        return None

    columns = [c[1] for c in conn.execute("PRAGMA table_info(items)")]
...
def main():
    parser = argparse.ArgumentParser(description="运行 items 同步检测")
    ...
    if args.file:
        ...
        suggestions = llm_extract_changes([stale], 'items', POKEMON_DB)
        ...
        out = os.path.join(os.path.dirname(__file__), 'sync_reports', f'sync_report_items_{timestamp}.json')
    ...
    # Phase 1: 新条目检测
    print(f'Detecting new items...', flush=True)
    new_entries, renamed_entries = detect_new_entries('items', pages, redirects, reverse_redirects, POKEMON_DB)
    ...
    new_items_info = []
    if new_entries:
        print(f'LLM extracting new items info...', flush=True)
        for i, entry in enumerate(new_entries):
            wiki_title = entry.get('wiki_title', '')
            print(f'  [{i+1}/{len(new_entries)}] {entry["name_zh"]}...', flush=True)
            info = llm_extract_entity_info(wiki_title, 'items')
            if info:
    ...
    if not args.new_only:
        print(f'Detecting stale items (since {args.since}, until {args.until or "now"})...', flush=True)
        stale = detect_stale_entries('items', pages, redirects, POKEMON_DB, args.since, args.until)
