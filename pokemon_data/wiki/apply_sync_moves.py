"""Apply moves sync changes from report to pokemonData.db"""
...
def find_latest_report():
    """找到最新的 moves 同步报告"""
    pattern = os.path.join(SYNC_REPORTS_DIR, 'sync_report_moves_*.json')
...
def apply_changes(report_path, dry_run=False):
    ...
    for entry in changes:
        ...
        cur.execute('SELECT id FROM moves WHERE name_en = ?', (name_en,))
        row = cur.fetchone()
        if not row:
        ...
        for change in entry['changes']:
            ...
            cur.execute(f'UPDATE moves SET {field} = ? WHERE id = ?', (new_val, move_id))
    ...
    conn.commit()
    conn.close()
    print(f'\nCommitted {updated} moves ({skipped} skipped)')
...
if __name__ == '__main__':
    ...
    if not report_path:
        ...
        if not report_path:
            print('未找到报告文件，请指定路径：python apply_sync_moves.py <report.json>')
            sys.exit(1)
